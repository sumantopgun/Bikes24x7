...
@section('title', 'Appointment Expire List')
@push('style')
...
<div class="content-wrapper">
    <div class="card">
        <div class="card-body">
        <h4 class="card-title" style="text-align: center;">Appointment Expire List</h4><br><br>
        @if (session('message'))
                ...
                <tbody>
                    @foreach ($expireBooking as $expireBook)
                        <tr>                       
                            ...
                            <td>Rs. {{ $expireBook->payments->payments_amount ?? 'N/A' }}</td>
                            ...
                            <td><a button class="btn btn-outline-primary" href="{{route('customercare.appointment.show', $expireBook->id)}}">Details</a></td>                                 
                        </tr>
                    @endforeach   
                </tbody>
                </table>                    
        ...
        @else
            <p class="alert alert-info">No Appointment Expire Found.....</p>
        @endif
        </div>
    </div>
</div>
